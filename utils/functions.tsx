...
export const searchString = (
  ...
  const searchResult =
    searchTarget instanceof Array
      ? !!searchTarget.filter((str) =>
          removeAccents(str).includes(removeAccents(searchKey)),
        ).length
...
export const validationRules = {
  ...
  onlyAlphabetic: (message: string): Rule => ({
    validator: (rule: any, value: string): Promise<void> => {
      if (!value || REGEX.alphabetic.test(removeAccents(value)))
        return Promise.resolve();
      return Promise.reject(
        message ||
          'This is a field where only alphabetic characters are entered !',
      );
    },
  }),
  onlyNumeric: (message: string): Rule => ({
    validator: (rule: any, value: string): Promise<void> => {
      ...
      return Promise.reject(
        message ||
          'This is a field where only numeric characters are entered !',
      );
    },
  }),
  onlyAlphanumerial: (message: string): Rule => ({
    validator: (rule: any, value: string): Promise<void> => {
      if (!value || REGEX.alphanumerial.test(removeAccents(value)))
        return Promise.resolve();
      return Promise.reject(
        message ||
          'This is a field where only alphanumeric characters are entered !',
      );
    },
  }),
...
export const formatAbbreviationsName = (value: string): string => {
  ...
  const firstLastWordFirstLetter = arrayString[arrayString.length - 2]
    .trim()
    .charAt(0);
  const secondLastWordFirstLetter = arrayString[arrayString.length - 1]
    .trim()
    .charAt(0);
...
export const formatISODateToDateTime = (
  time: string,
  format?: string,
): string => moment(time).format(format || EFormat.DATE_TIME);
...
export const handleErrorImageUrl = (
...
export const truncateStringByCharaters = (
  content: string,
  maxLength: number,
): string => {
...
export const getPages = (
  ...
  const lastPages = pages.filter(
    (_, i) => i > pages.length - numberOfPages - 1,
  );
  const numberOfPagesAroundASide = (numberOfPages - 1) / 2;
  const pagesAtHead = pages.filter((_, i) => i < numberOfPagesAroundASide);
  const pagesAtTail = pages.filter(
    (_, i) => i > pages.length - numberOfPagesAroundASide - 1,
  );
...
export const splitArrayPerChunk = (
...
export const formatCurrency = (config: {
  ...
  const separateMoney = Intl.NumberFormat('vi-VN').format(
